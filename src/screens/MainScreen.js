...
const MainScreen = () => {
  ...
  const [needsRescue,
    setNeedsRescue] = useState(false);
  const [locationState,
    setLocationState] = useState(false);
  const [active,
    setActive] = useState(false);
  const [isSendingSosAlert,
    setIsSendingSosAlert] = useState(false);
  const [userSpeed,
    setUserSpeed] = useState(0);
  const [shipMarkersActive,
    setShipMarkersActive] = useState(true);

  const [collisionDetected,
    setCollisionDetected] = useState(false);
  const [userWithinRadius,
    setUserWithinRadius] = useState([]);
    ...
    if (location) {
      ...
      try {
        ...
        await firebase
          ...
          .get()
          .then((res) => {
            locationData.boatName = res
              .data()
              .boatName;
            locationData.boatType = res
              .data()
              .boatType;
          })
          .catch((err) => {
  ...
  const sosAlert = () => {
    Alert.alert('SOS Alert', 'People nearby will receive your alert', [
      {
        text: 'Rescued',
  ...
  const receiveSosAlert = (data) => {
    Alert.alert('SOS Alert', data[0].username + ' needs your help', [
      {
        text: 'Accept',
        onPress: () => {
  ...
  const sendSosConfirm = () => {
    Alert.alert('SOS Alert', 'Do you wish to send SOS Alert', [
      {
        text: 'Yes',
      ...
      <View style={styles.mapContainer}>
        <MapView
          ...
          {shipMarkers.map((res, i) => {
            ...
            const vesselIcon = res.shipType > 60
              ? isDarkTheme
            ...
            if (shipMarkersActive === true) {
              return (<Marker
                ...
                .mmsi
                .toString()}
                description={`${ (currentTime - res.properties.timestampExternal) / 1000}s ago, shiptype: ${res.shipType}, ship name: ${res.name}`}
                image={vesselIcon}/>);
            }
          ...
          {userMarkers.map((res, i) => {
            ...
            if (firebase.auth().currentUser.uid !== res.uid) {
              const icon = res.needsRescue === true
